<script>
(function () {
  // 1. 样式（自动根据 html.dark / html.light 调整字体颜色）
...
html.dark #music-player button,
html.dark #song-info {
  color: white;
}
html.light #music-player,
html.light #music-player button,
html.light #song-info {
  color: black;
}
...
`;
  ...
  function initAudio() {
    ...
    audio.addEventListener('play', () => { isPlaying = true; updateButtons(); });
    audio.addEventListener('pause', () => { isPlaying = false; updateButtons(); });
  }
  ...
  window.addEventListener('load', () => {
    ...
    updateSongInfo();
    updateButtons();
    loadAndPlay(false); // 初次不自动播放，避免浏览器限制
  });
